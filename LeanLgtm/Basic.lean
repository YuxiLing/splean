...
by
  apply himpl_antisym
  { move=> ? ![?? hEmpty *]
    sby apply hempty_inv in hEmpty }
  move=> h ?
  exists ∅, h
  repeat' (constructor=>//)
...
lemma qwand_specialize A (x : A) (Q1 Q2 : A → hprop) :
  (Q1 -∗∗ Q2) ==> (Q1 x -∗ Q2 x) :=
by
  sby apply (himpl_hforall_l A x)=> h
...
def triple (t : trm) (H : hprop) (Q : val → hprop) : Prop :=
