import Mathlib.Data.Set.Card
import Mathlib.Data.Real.Sqrt

import Lgtm.Hyper.ProofMode
import Lgtm.Hyper.ArraysFun

import Lgtm.Experiments.HyperCommon

open Unary prim val trm
open Classical

notation "⋆" => (Set.univ)

@[app_unexpander Set.univ]
def  unexpandUniv : Lean.PrettyPrinter.Unexpander
  | `($_) => `(⋆)

def val.toBool : val -> Bool
  | val_bool v => v
  | _ => panic! "toBool"

def val.toNat : val -> Nat
  | val_int v => v.toNat
  | _ => 1

@[simp]
noncomputable def propToVal : Prop -> val := fun p => val_bool (decide p)

lemma val_bool_eq_propToVal :
  val_bool b = propToVal p ↔ b = p := by
  simp

noncomputable instance : Coe Prop val := ⟨propToVal⟩

@[simp]
lemma toBool_eq (b : Bool) : val.toBool b = b := by rfl

-- instance : Coe val Prop := ⟨fun v => v.toBool⟩

@[app_unexpander val.toBool] def unexpandToProp : Lean.PrettyPrinter.Unexpander
  | `($_ $v) => `($v)
  | _ => throw ( )

attribute [-simp] fun_insert Bool.forall_bool
attribute [simp] Set.univ_inter

theorem biUnion_prod_const (s': Set ι) {s : ι → Set α} {t : Set β} :
  (⋃ i ∈ s', s i) ×ˢ t = ⋃ i ∈ s', s i ×ˢ t := by
  ext
  simp=> ⟨|⟩ ![] // ![] //

-- FIXME: temp move here
def toProp : val -> Prop
  | val_bool v => v
  | _ => False

instance : Coe val Prop := ⟨(·.toBool = true)⟩

/-   -----
lang_def Lang.getPoints :=
  fun pxind pyind i j =>
    let N := len pxind in
    let ind := Lang.find2Idx pxind pyind i j 0 N in
    ind != N

lang_def Lang.inBox :=
  fun xind yind boxl boxr =>
    let boxlx := boxl[0] in
    let boxly := boxl[1] in
    let boxrx := boxr[0] in
    let boxry := boxr[1] in

    let xinl := boxlx <= xind in
    let xinr := xind < boxrx in
    let xin := xinl && xinr in

    let yinl := boxly <= yind in
    let yinr := yind < boxry in
    let yin := yinl && yinr in

    xin && yin

lang_def Lang.getBox := fun boxl boxr i j => Lang.inBox i j boxl boxr


lang_def Lang.beforeBox :=
  fun xind yind boxl boxr =>
    let boxrx := boxr[0] in
    let boxry := boxr[1] in

    let xinr := xind < boxrx in
    let yinr := yind < boxry in

    xinr && yine


lang_def Lang.boxCountQuery :=
  fun pxind pyind boxl boxr  =>
    ref ans := 0 in
    let N := len pxind in
    for i in [0:N] {
      let xind := pxind[i] in
      let yind := pyind[i] in
      let InBox := Lang.inBox xind yind boxl boxr in
      if InBox then
        incr ans
      else ()
    }; !ans

namespace BoxCount
variable (xind yind boxl boxr : loc) (N : ℕ)
variable (x_ind : ℤ -> ℝ) (y_ind : ℤ -> ℝ) (box_l box_r : ℤ -> ℝ)

#hint_yapp triple_hharrayFun_get
#hint_yapp triple_hharrayFun_length
#hint_yapp htriple_eq
#hint_yapp htriple_neq
#hint_yapp htriple_get
-- #hint_yapp htriple_free
#hint_yapp htriple_sub
#hint_yapp htriple_ler
#hint_yapp htriple_ltr
#hint_yapp htriple_add
#hint_yapp htriple_ptr_add



variable (xy_ind_uniq : Set.InjOn (fun i => (x_ind i, y_ind i)) ⟦0, N⟧)
include xy_ind_uniq

lemma get_spec_out (l : ℕ) (s : Set (ℝ × ℝ)) :
  (∀ᵉ (i ∈ s) (j ∈ ⟦0,N⟧), ¬ (x_ind j = i.1 ∧ y_ind j = i.2)) ->
  arr⟨⟪l,s⟫⟩(xind, x in N => x_ind x) ∗
  arr⟨⟪l,s⟫⟩(yind, x in N => y_ind x) ==>
    WP [l| ij in s => Lang.getPoints xind yind ⟨ij.val.1⟩ ⟨ij.val.2⟩] { v,
     ⌜ v = fun _ => val_bool false ⌝ ∗
     arr⟨⟪l,s⟫⟩(xind, x in N => x_ind x) ∗
     arr⟨⟪l,s⟫⟩(yind, x in N => y_ind x) } := by
  move=> nin
  ystep; ywp; yapp Lang.find2Idx_hspec_out
  { yapp; ysimp }
  move=> ??; apply nin=> //

notation "·" => (Set.univ)

def toProp : val -> Prop
  | val_bool v => v
  | _ => False

instance : Coe val Prop := ⟨(·.toBool = true)⟩

-- @[app_unexpander toProp] def unexpandToProp : Lean.PrettyPrinter.Unexpander
--   | `($_ $v) => `($v)
--   | _ => throw ( )

attribute [-simp] hhwandE
lemma get_spec_in (l : ℕ) (k : ℤ) :
  0 <= k ∧ k < N ->
  arr⟨⟪l,{(x_ind k, y_ind k)}⟫⟩(xind, x in N => x_ind x) ∗
  arr⟨⟪l,{(x_ind k, y_ind k)}⟫⟩(yind, x in N => y_ind x) ==>
    WP [l| ij in {(x_ind k, y_ind k)} => Lang.getPoints xind yind ⟨ij.val.1⟩ ⟨ij.val.2⟩] { v,
     ⌜ v = fun _ => val_bool true ⌝ ∗
    arr⟨⟪l,{(x_ind k, y_ind k)}⟫⟩(xind, x in N => x_ind x) ∗
    arr⟨⟪l,{(x_ind k, y_ind k)}⟫⟩(yind, x in N => y_ind x) } := by
  move=> ?
  ystep; simpWP; ywp; yapp Lang.find2Idx_hspec
  yapp; ysimp=> /== //'; funext=> /== //'

set_option maxRecDepth 1500
set_option maxHeartbeats 3200000

def InBox x y := (box_l 0 <= x ∧ x < box_r 0) ∧ (box_l 1 <= y ∧ y < box_r 1)

omit xy_ind_uniq in
@[yapp]
lemma inBox_spec' (f₁ f₂ : (ℝ×ℝ)ˡ -> ℝ) :
  arr⟨⋆⟩(boxl, x in 2 => box_l x) ∗
  arr⟨⋆⟩(boxr, x in 2 => box_r x) ==>
  hwp ⟪l, r⟫ (fun ij => [lang| Lang.inBox ⟨f₁ ij⟩ ⟨f₂ ij⟩ boxl boxr]) fun v =>
    ⌜ v = fun ij => propToVal (InBox box_l box_r (f₁ ij) (f₂ ij)) ⌝ ∗
    arr⟨⋆⟩(boxl, x in 2 => box_l x) ∗
    arr⟨⋆⟩(boxr, x in 2 => box_r x) := by
  sdo 10 (ystep=> //'); yapp; ysimp=> /==
  funext ⟨i, j⟩; simp [InBox]

omit xy_ind_uniq in
lemma inBox_spec (i j : ℝ) :
  { arr⟨⋆⟩(boxl, x in 2 => box_l x) ∗
    arr⟨⋆⟩(boxr, x in 2 => box_r x) }
  [1| x in {r} => Lang.inBox i j boxl boxr]
  [2| ij in @Set.univ (ℝ × ℝ) => Lang.getBox boxl boxr ⟨ij.val.1⟩ ⟨ij.val.2⟩]
  {v,
    ⌜ v ⟨1,r⟩ = ((i, j) ∈ {ij | v ⟨2, ij⟩ }) ⌝ ∗
    arr⟨⋆⟩(boxl, x in 2 => box_l x) ∗
    arr⟨⋆⟩(boxr, x in 2 => box_r x) } := by
  yin 1: yapp
  ystep; ysimp



lemma boxSerachQuery_spec :
  { arr⟨⋆⟩(boxl, x in 2 => box_l x) ∗
    arr⟨⋆⟩(boxr, x in 2 => box_r x) ∗
    arr⟨⋆⟩(xind, x in N => x_ind x) ∗
    arr⟨⋆⟩(yind, x in N => y_ind x) }
  [1| x in {r} => Lang.boxCountQuery xind yind boxl boxr]
  [2| ij in @Set.univ (ℝ × ℝ) => Lang.getPoints xind yind ⟨ij.val.1⟩ ⟨ij.val.2⟩]
  [3| ij in @Set.univ (ℝ × ℝ) => Lang.getBox boxl boxr ⟨ij.val.1⟩ ⟨ij.val.2⟩]
  {v,
    ⌜ (v ⟨1,r⟩).toNat = ({ij | v ⟨2, ij⟩ } ∩ {ij | v ⟨3, ij⟩ }).encard ⌝  ∗ ⊤ } := by
  yin 3: ystep
  yfocus 2, (fun i => (x_ind i, y_ind i)) '' ⟦0, N⟧
  yapp get_spec_out=> /==; simp [fun_insert]
  yin 1: ywp; yref p; ystep
  srw [1]Set.image_eq_iUnion
  yfor+ with
    (Q := fun i hv => p ~⟪1, {r}⟫~> val_int (if InBox box_l box_r (x_ind i) (y_ind i) then 1 else 0) ∗ ⌜hv ⟨2, x_ind i, y_ind i⟩⌝ )
    (H₀ := p ~⟪1, {r}⟫~> 0);
  { move=> > ??
    yin 1: ystep=> //'; ystep=> //'; ystep; ywp; yif=> /== cnd
    { yin 1: yapp
      yapp get_spec_in; ysimp; srw if_pos // }
    yin 1: ywp; yval
    yapp get_spec_in; ysimp }
  { sdone }
  yapp=> hvIn;
  ysimp=> /==
  erw [<-Finset.sum_image (g := fun i => (x_ind i, y_ind i)) (f := fun i => if InBox _ _ i.1 i.2 then 1 else 0)]
  erw [Finset.sum_ite, Finset.sum_const_zero]; simp [-Finset.mem_Ico, toNat]=> //'
  srw -Set.ncard_coe_Finset Set.Finite.cast_ncard_eq
  { congr! => ! [r₁ r₂] /== ? ⟨xin|⟩
    { srw if_pos //; move: xin=> /== ??? <-<- //' }
    scase_if=> // }
  apply Finset.finite_toSet

end BoxCount
-/
namespace IntervalCount
set_option maxRecDepth 1500
set_option maxHeartbeats 3200000

-- FIXME: temp move here
#hint_yapp triple_hharrayFun_get
#hint_yapp triple_hharrayFun_length
#hint_yapp htriple_eq
#hint_yapp htriple_neq
#hint_yapp htriple_get
-- #hint_yapp htriple_free
#hint_yapp htriple_sub
#hint_yapp htriple_ler
#hint_yapp htriple_ltr
#hint_yapp htriple_add
#hint_yapp htriple_ptr_add

#hint_yapp htriple_addr
#hint_yapp htriple_subr
#hint_yapp htriple_mulr
#hint_yapp htriple_lt

@[app_unexpander hharrayFun]
def hharrayFunUnexpander : Lean.PrettyPrinter.Unexpander
  | `($_ $s $f $n fun $_ => $p) =>
    match s with
    | `(Set.univ) => `(⋯)
    | `(⋆) => `(⋯)
    | _ =>
      match f with
      | `(fun $x:ident => $f) =>
        `(arr($p ⟨$s:term⟩ , $x:ident in $n => $f))
      | _ => throw ( )
  | _ => throw ( )

-- lang_def Lang.inInt :=
--   fun i intl intr =>
--     let iLl := intl <= i in
--     let iLr := i < intr in
--     iLl && iLr

-- lang_def Lang.intervalCountQuery :=
--   fun xind xid intl intr out =>
--     let N := len xind in
--     for i in [0:N] {
--       let ind := xind[i] in
--       let id  := xid[i] in
--       let InInt := Lang.inInt ind intl intr in
--       if InInt then
--         let out' := out ++ 1 in
--         let out' := out' ++ id in
--         Lang.incr out'
--       else ()
--     }; ()

/-
NOTE:
The circle area is represented by its center and its radius.
Variable naming: center: `cx`, `cy`; radius: `rad`.

Compared with the box case, the valid `x`-coordinate range changes
with the `y`-coordinate. The `x`-coordinate range is
`[ cx - √(rad² - (y - cy)²) , cx + √(rad² - (y - cy)²) ]`.

Design: since the language does not have `√` as a primitive, we use
a dedicated subprocedure for checking whether a point is inside the circle.
In this case, we only need to use multiplication and addition.

As a side effect, the break condition of the while-loop is not
as simple as the box case; it seems that some repeated computation is necessary.
See the details below.

**Choice**: For now, the points on the circle are considered inside the circle.
-/

-- usually `rem = rad² - (y - cy)²`
lang_def Lang.onChord :=
  fun x cx rem =>
    let dx := x - cx in
    let dx2 := dx * dx in
    let inCircle := dx2 <= rem in
    inCircle

lang_def Lang.onChordOrLeft :=
  fun x cx rem =>
    let cond1 := x <= cx in
    let cond2 := onChord x cx rem in
    let res := cond1 || cond2 in
    res

-- FIXME: changing `abbrev` to `def`?
abbrev openChordRange (cx rem : ℝ) := Set.Iic (cx + √rem)

abbrev closedChordRange (cx rem : ℝ) := if rem < 0 then ∅ else Set.Icc (cx - √rem) (cx + √rem)

lemma open_closed_ChordRange x cx rem : x ≤ cx ∨ x ∈ closedChordRange cx rem ↔ x ∈ openChordRange cx rem := by
  simp [closedChordRange, openChordRange]
  have ha := Real.sqrt_nonneg rem
  constructor <;> intro h ; rcases h with h | h
  all_goals (try linarith)
  by_cases h' : x ≤ cx ; left ; assumption
  simp at h' ; by_cases h'' : 0 ≤ rem
  { right ; split_ands <;> linarith }
  { srw Real.sqrt_eq_zero_of_nonpos // at h ; linarith }

lemma closedChordRange_implies_open x cx rem : x ∈ closedChordRange cx rem → x ∈ openChordRange cx rem := by
  simp [closedChordRange, openChordRange]

-- `simp` will simplify `closedChordRange` into `0 ≤ rem ∧ cx ≤ x + √rem ∧ x ≤ cx + √rem`
lemma closedChordRange_simp x cx rem : x ∈ closedChordRange cx rem ↔ 0 ≤ rem ∧ cx ≤ x + √rem ∧ x ≤ cx + √rem := by
  simp [closedChordRange]

lemma closedChordRange_meaning x cx rem : x ∈ closedChordRange cx rem ↔ ((x - cx) ^ 2 ≤ rem) := by
  simp [closedChordRange]
  constructor
  · intro ⟨ha, hb, hc⟩
    rw [Real.sq_le ha]
    constructor <;> linarith
  · intro h
    by_cases hh : 0 ≤ rem
    · rw [Real.sq_le hh] at h
      split_ands <;> linarith
    · simp_all
      nlinarith

-- def insideRangeProp (center rad x : ℝ) := (center - rad <= x ∧ x <= center + rad)

section

-- FIXME: why this is not in [simp]?
lemma selfmul_is_square (x : ℝ) : x * x = x ^ 2 := by ring

-- @[yapp]
-- lemma onChord_spec (f₁ f₂ f₃ : (ℝ×ℝ)ˡ -> ℝ) :
--   emp ==>
--   hwp ⟪l, r⟫ (fun ij => [lang| Lang.onChord ⟨f₁ ij⟩ ⟨f₂ ij⟩ ⟨f₃ ij⟩]) fun v =>
--     ⌜ v = fun ij => propToVal ((f₁ ij - f₂ ij) ^ 2 ≤ (f₃ ij)) ⌝ := by
--   ystep ; ystep ; ystep
--   yval    -- ?
--   ysimp
--   funext ⟨i, j⟩; simp [insideRangeProp, selfmul_is_square]

@[yapp]
lemma onChord_spec_single (dummy : α) (x cx rem : ℝ) :
  emp ==>
  hwp ⟪l, { dummy }⟫ (fun _ => [lang| Lang.onChord x cx rem]) fun v =>
    ⌜ v = fun _ => propToVal ((x - cx) ^ 2 ≤ rem) ⌝ := by
  sdo 3 ystep
  yval    -- ?
  ysimp
  funext; simp [selfmul_is_square]

@[yapp]
lemma onChordOrLeft_spec_single (dummy : α) (x cx rem : ℝ) :
  emp ==>
  hwp ⟪l, { dummy }⟫ (fun _ => [lang| Lang.onChordOrLeft x cx rem]) fun v =>
    ⌜ v = fun _ => propToVal (x ∈ openChordRange cx rem) ⌝ := by
  srw -open_closed_ChordRange closedChordRange_meaning
  sdo 3 ystep
  yval    -- ?
  ysimp

-- @[yapp]
-- lemma onChord_spec (cx : ℝ) (f₁ f₂ : (ℝ×ℝ)ˡ -> ℝ) (hnneg : ∀ ij, 0 <= f₂ ij) :
--   emp ==>
--   hwp ⟪l, r⟫ (fun ij => [lang| Lang.onChord ⟨f₁ ij⟩ cx ⟨f₂ ij⟩]) fun v =>
--     ⌜ v = fun ij => propToVal (insideRangeProp cx √(f₂ ij) (f₁ ij)) ⌝ := by
--   ystep ; ystep ; ystep
--   yval    -- ?
--   ysimp
--   funext ⟨i, j⟩; simp only [insideRangeProp, selfmul_is_square]
--   congr   -- ?
--   ext ; srw Real.sq_le=> //
--   constructor=> [] /== ?? ⟨|⟩ <;> linarith

end

lang_def Lang.intervalCountQuery' :=
  fun xind xid cx rem out z n =>
    ref cnt := z in
    while (
      let i := !cnt in
      let inBounds := i < n in
      if inBounds then
        let x := xind[i] in
        let res := onChordOrLeft x cx rem in
        res
      else false) {
      let i := !cnt in
      let x := xind[i] in
      let cond := onChord x cx rem in
      if cond then
        let id := xid[i] in
        let out' := out ++ 1 in
        let out' := out' ++ id in
        Lang.incr out';
        Lang.incr cnt
      else Lang.incr cnt
    }

-- NOTE: the same as in the box case
lang_def Lang.mem :=
  fun xind xid i id z n =>
    let n' := Lang.find2Idx xind xid i id z n in
    n' != n

variable (xind xid : loc) (z n : ℤ) (N : ℕ) (M : ℕ)
variable (x_ind : ℤ -> ℝ) (x_id : ℤ -> ℤ) (out : loc)
variable (x_id_range : x_id '' ⟦z, n⟧ ⊆ Set.Ico 0 M)
variable (zLn : z <= n) (oLz : 0 <= z) (nLN : n <= N)

section
variable (x_ind_inj : Set.InjOn x_ind ⟦z, n⟧)

include x_ind_inj zLn oLz nLN

lemma mem_spec_out (l : ℕ) (s : Set (ℝ × ℤ)) :
  (∀ᵉ (i ∈ s) (j ∈ ⟦z,n⟧), ¬ (x_ind j = i.1 ∧ x_id j = i.2)) ->
  arr⟨⟪l,s⟫⟩(xind, x in N => x_ind x) ∗
  arr⟨⟪l,s⟫⟩(xid, x in N => x_id x) ==>
    WP [l| ij in s => Lang.mem xind xid ⟨ij.val.1⟩ ⟨ij.val.2⟩ z n] { v,
     ⌜ v = fun _ => val_bool false ⌝ ∗
     arr⟨⟪l,s⟫⟩(xind, x in N => x_ind x) ∗
     arr⟨⟪l,s⟫⟩(xid, x in N => x_id x) } := by
  move=> nin
  ywp; yapp Lang.find2Idx_hspec_out'
  { yapp; ysimp }
  move=> ??;
  { move=> ?? /= /== /x_ind_inj // }
  apply nin=> //


attribute [-simp] hhwandE
lemma mem_spec_in (l : ℕ) (k : ℤ) :
  z <= k ∧ k < n ->
  arr⟨⟪l,{(x_ind k, x_id k)}⟫⟩(xind, x in N => x_ind x) ∗
  arr⟨⟪l,{(x_ind k, x_id k)}⟫⟩(xid, x in N => x_id x) ==>
    WP [l| ij in {(x_ind k, x_id k)} => Lang.mem xind xid ⟨ij.val.1⟩ ⟨ij.val.2⟩ z n] { v,
     ⌜ v = fun _ => val_bool true ⌝ ∗
    arr⟨⟪l,{(x_ind k, x_id k)}⟫⟩(xind, x in N => x_ind x) ∗
    arr⟨⟪l,{(x_ind k, x_id k)}⟫⟩(xid, x in N => x_id x) } := by
  move=> ?
  simpWP; ywp; yapp Lang.find2Idx_hspec'
  { yapp; ysimp=> /== //'; funext=> /== //' }
  move=> ???? /= /== /x_ind_inj //

omit x_ind_inj in
lemma mem_spec_term (l : ℕ) (s : Set α) (x : αˡ -> ℝ) (y : αˡ -> ℤ) :
  -- (∀ᵉ (i ∈ s) (j ∈ ⟦z,n⟧), ¬ (x_ind j = i.1 ∧ x_id j = i.2)) ->
  arr⟨⟪l,s⟫⟩(xind, x in N => x_ind x) ∗
  arr⟨⟪l,s⟫⟩(xid, x in N => x_id x) ==>
    WP [l| ij in s => Lang.mem xind xid ⟨x ij⟩ ⟨y ij⟩ z n] { v,
     arr⟨⟪l,s⟫⟩(xind, x in N => x_ind x) ∗
     arr⟨⟪l,s⟫⟩(xid, x in N => x_id x) } := by
  srw ?hharrayFun ?bighstar_hhstar; apply htriple_prod (Q := fun _ _ => _)=> a ?
  xwp; xapp Lang.find2Idx_spec_term
  xwp; xapp

-- omit x_ind_inj in
-- @[yapp]
-- lemma mem_inte_in (i : ℝ) :
--   emp ==>
--     WP [l| ij in s => Lang.inInt i intl intr ] { v,
--      ⌜ v = fun _ => propToVal (i ∈ Set.Ico intl intr) ⌝ } := by
--   ystep; ystep; yapp

include x_id_range

-- lemma intervalCountQuery_spec :
--   { arr⟨⋆⟩(xind, x in N => x_ind x) ∗
--     arr⟨⋆⟩(xid,  x in N => x_id x) ∗
--     arr⟨⟪1, {r}⟫⟩(out,  x in M => val_int 0) }
--   [1| x in {r} => Lang.intervalCountQuery xind xid intl intr out]
--   [2| ij in @Set.univ (ℝ × ℤ) => Lang.mem xind xid ⟨ij.val.1⟩ ⟨ij.val.2⟩]
--   {v,
--     arr⟨⟪1, {r}⟫⟩(out, id in M => val_int ({i | v ⟨2,i,id⟩} ∩ Set.Ico intl intr).ncard)  ∗ ⊤ } := by
--   yfocus 2, (fun i => (x_ind i, x_id i)) '' ⟦0, N⟧
--   yapp mem_spec_out=> /==; simp [fun_insert]
--   yin 1: ystep
--   srw [1]Set.image_eq_iUnion
--   let op (hv : hval (ℝ×ℤ)ˡ) i := if hv ⟨2, x_ind i, x_id i⟩ ∧ intl <= x_ind i ∧ x_ind i < intr then (1 : ℤ) else 0
--   yfor+ with
--     (Q := fun j hv => out + 1 + (x_id j).natAbs ~⟪1, {r}⟫~> op hv j)
--     (H₀ := arr⟨⟪1, {r}⟫⟩(out,  x in M => val_int 0) ∗ ⌜x_id '' ⟦0, N⟧ ⊆  Set.Ico 0 M⌝) <;> try simp [op]
--   { move=> > ?? -> // }
--   { move=> > ??
--     shave ?: x_id i >= 0
--     { simp: (@x_id_range (x_id i))=> /(_ i); omega }
--     yin 2: yapp mem_spec_in
--     simp; (sdo 3 ystep=> //'); ywp; yif=> [/== ??|/== cnd]
--     { ystep=> //' /==; ystep=> //'
--       shave->: ((((Nat.cast out) + 1) : ℤ).natAbs + x_id i).natAbs = out + 1 + (x_id i).natAbs;
--       { move: out; unfold loc; omega }
--       yapp; ysimp; srw if_pos //' }
--     ywp; yval; ysimp }
--   { ysimp=> // }
--   ysimp=> _; ywp; yval
--   open AddPCM in erw [<-Finset.sum_image
--        (g := fun i => (x_ind i, x_id i))
--        (f := fun i => _ + i.2.natAbs ~⟪1, _⟫~> val_int (if (hv ⟨2, i⟩).toBool = _ ∧ _ ≤ i.1 ∧ i.1 < _ then 1 else 0))]
--   { open AddPCM in srw arr_eq_sum=> //'
--     { apply boo=> /== i ??; srw -Set.ncard_coe_Finset /==
--       apply Set.ncard_congr (f := fun x _ => x.1)=> /== //'
--       { move=> > ?? <- <- <-; srw if_pos //'; exists x=> //' }
--       move=> >; scase_if=> /== > ?? <- <- ???; exists (x_id x)=> ⟨⟨|//'⟩|//'⟩
--       exists x=> //' }
--     move=> i /==; move: (@x_id_range i)=> /== //' }
--   move=> ? /x_ind_inj /== H ??? /H //'

end

section

variable (x_ind_inj : StrictMonoOn x_ind ⟦z, n⟧)
include x_ind_inj x_id_range zLn oLz nLN

variable (cx rem : ℝ)
include cx rem

lemma intervalCountQuery'_spec (ini : ℤ -> ℤ) :
  { arr⟨⋆⟩(xind, x in N => x_ind x) ∗
    arr⟨⋆⟩(xid,  x in N => x_id x) ∗
    arr⟨⟪1, {r}⟫⟩(out,  x in M => ini x) }
  [1| x in {r} => Lang.intervalCountQuery' xind xid cx rem out z n]
  [2| ij in @Set.univ (ℝ × ℤ) => Lang.mem xind xid ⟨ij.val.1⟩ ⟨ij.val.2⟩ z n]
  {v,
    ⌜∀ id ∈ ⟦0,M⟧, ({i | v ⟨2,i,id⟩} ∩ (closedChordRange cx rem)).Finite⌝ ∗
    arr⟨⟪1, {r}⟫⟩(out, id in M => val_int $ ini id + ({i | v ⟨2,i,id⟩} ∩ (closedChordRange cx rem)).ncard) ∗
    arr⟨⋆⟩(xind, x in N => x_ind x) ∗
    arr⟨⋆⟩(xid,  x in N => x_id x) } := by
  shave Inj: Set.InjOn x_ind (Set.Ico z n)
  { apply StrictMonoOn.injOn
    move=>  /== ??? ??? /x_ind_inj; sapply=> /== //' }
  yfocus 2, (fun i => (x_ind i, x_id i)) '' ⟦z, n⟧
  yapp mem_spec_out=> /==; simp [fun_insert]
  yin 1: ywp; yref ind
  srw [1]Set.image_eq_iUnion
  let op (hv : hval (ℝ×ℤ)ˡ) i := if hv ⟨2, x_ind i, x_id i⟩ ∧ (x_ind i ∈ closedChordRange cx rem) then (1 : ℤ) else 0
  let cond (i : ℤ) := i < n ∧ (x_ind i ∈ openChordRange cx rem)
  ywhile+ z, n with (b₀ := cond z)
    (Inv := fun b i => (if cond i then ind ~⟪1, {r}⟫~> i else ∃ʰ (j : ℤ), (ind ~⟪1, {r}⟫~> j) ∗ ⌜z <= j ∧ ¬ cond j⌝) ∗ ⌜0 <= i ∧ i <= n ∧ cond i = b⌝ )
    (Q := fun j hv => out + 1 + (x_id j).natAbs ~⟪1, {r}⟫~> op hv j)
    (H₀ := arr⟨⟪1, {r}⟫⟩(out,  x in M => ini x) ∗ ⌜x_id '' ⟦z, n⟧ ⊆  Set.Ico 0 M⌝) <;> try simp [op]
  { move=> ?; split_ifs; simp [hhlocalE]; srw hhlocal_hhexists; simp [hhlocalE] }   -- ?
  { move=> > ?? -> // }
  { move=> > ??
    shave ?: x_id j >= 0
    { simp: (@x_id_range (x_id j))=> /(_ j); omega }
    yin 2: yapp mem_spec_in=> [_ [_ [_ hcond]]]
    srw if_pos //'
    simp; sdo 3 ystep
    ywp; yif=> /== hifcond
    { srw -closedChordRange_meaning at hifcond; simp at hifcond; srw if_pos //'; ystep=> //'; ystep=> //' /==; ystep=> //'
      shave->: ((((Nat.cast out) + 1) : ℤ).natAbs + x_id j).natAbs = out + 1 + (x_id j).natAbs;
      { move: out; unfold loc; omega }
      ystep
      yapp;
      ysimp[decide (cond (j + 1))]=> /== //'
      split_ifs=> /== * <;> ysimp; move=> ⟨|⟩ //' }  -- ? scase_if
    have hcontra : ¬ x_ind j ∈ closedChordRange cx rem := by srw closedChordRange_meaning ; linarith  -- ? if_neg
    srw closedChordRange_simp at hcontra ; srw hcontra=> /=
    yapp; ysimp[decide (cond (j + 1))]=> /== //'
    split_ifs <;> ysimp; move=> ⟨|⟩ //' } -- ? scase_if
  { move=> > ? jN; srw LGTM.triple ywp1
    yapp mem_spec_in=> /== _ _; srw /== => hcond; srw hcond /=
    have hq : ¬ cond (j + 1) := by
      simp [cond] at hcond ⊢
      move=> h1; apply lt_trans; apply hcond=> //; apply x_ind_inj=> /== //'
    srw hq /==
    srw if_neg ; rotate_left ; intro hh ; apply hcond ; simp [cond] ; split_ands <;> linarith
    ypull=> i ?; ysimp=> //' }
  { move=> > ??; split_ifs <;> rename_i L  -- ? scase_if
    { ystep=> /== ?? hci; srw hci at L; subst b; simp at hci ; srw cond at hci
      ystep; ywp; yif=> /== ?
      { sdo 2 ystep; yval; ysimp=> //'  -- ? yval
        funext=> /==; aesop }
      ywp; yval; ysimp=> //' }
    srw -hwp_equiv; ypull=> i /== ? hh ?? hci; srw hci at L; simp at L; subst b
    sdo 2 ystep; ywp; yif=> /== ?
    { sdo 2 ystep; yval; ysimp=> //'  -- ? yval
      funext=> /==; aesop }
    ywp; yval; ysimp=> //' }
  { move=> ?; srw cond /==; ysimp=> //' }
  { ysimp=> //; split_ifs <;> ysimp=> //' } -- ? scase_if

  open AddPCM in erw [<-Finset.sum_image
    (g := fun i => (x_ind i, x_id i))
    (f := fun i => _ + i.2.natAbs ~⟪1, _⟫~> val_int (if (v ⟨2, i⟩).toBool = _ ∧ 0 ≤ rem ∧ cx ≤ i.1 + √rem ∧ i.1 ≤ cx + √rem then 1 else 0))]
  rotate_left
  { move=> ?? ?? /== /Inj /[swap] ?; sapply=> // }
  open AddPCM in srw arr_eq_sum=> //'; rotate_left
  { move=> i /==; move: (@x_id_range i)=> /== //' }
  ypull=> _ [_ hn]; ysimp[False]
  { move=> /== i ??; srw -Set.ncard_coe_Finset /==
    apply Set.ncard_congr (f := fun x _ => x.1)=> /== //'
    { move=> > ?? <- <- <-; srw if_pos //'; exists x=> //' }
    move=> >; scase_if=> /== > ?? <- <- ????; exists (x_id x)=> ⟨⟨|//'⟩|//'⟩
    exists x=> //' }
  { move=> id > ??
    srw (Equiv.set_finite_iff (t :=
        (Finset.filter (fun x ↦ (v ⟨2, x⟩).toBool = true ∧ 0 ≤ rem ∧ cx ≤ x.1 + √rem ∧ x.1 ≤ cx + √rem)
         (Finset.filter (fun x ↦ x.2 = id) (Finset.image (fun i ↦ (x_ind i, x_id i)) (Finset.Ico z n)))).toSet))=> //'
    apply Finset.finite_toSet
    apply Equiv.mk
      (toFun := fun ⟨x, pf⟩ => ⟨(x,id),
        by simp: pf; scase_if=> /== > ?? <- <- ???? ⟨|⟩//'⟩)
      (invFun := fun ⟨⟨x, id⟩ , pf⟩ => ⟨x,
        by simp: pf=> > ?? <- <- <-; srw if_pos //'; exists x=> //'⟩)
    { move=> ? /== }
    move=> /== // }
  srw hn /= ; ysimp=> j hj
  apply (hhimpl_hhexists_r (fun _ ↦ val_int j))=> //    -- ?

end

end IntervalCount

open IntervalCount

lang_def Lang.circleCountQuery' :=
  fun yprt yind xind xid cx cy rad out =>
    let N := len yind in
    let intl := cy - rad in
    let intr := cy + rad in
    let rad2 := rad * rad in
    ref cnt := 0 in
    while (
      let i := !cnt in
      let inBounds := i < N in
      if inBounds then
        let ind := yind[i] in
        ind <= intr
      else false) {
      let i := !cnt in
      let y := yind[i] in
      let indLr := intl <= y in
      if indLr then
        let z := yprt[i] in
        let i' := i + 1 in
        let n := yprt[i'] in
        let dy := y - cy in
        let dy2 := dy * dy in
        let rem := rad2 - dy2 in
        Lang.intervalCountQuery' xind xid cx rem out z n;
        Lang.incr cnt
      else Lang.incr cnt
    }

lang_def Lang.mem2 :=
  fun yprt yind xind xid i j id =>
    let N := len yind in
    let n := Lang.findIdx yind i 0 N in
    let inBounds := n != N in
    if inBounds then
      let z := yprt[n] in
      let i' := n + 1 in
      let n := yprt[i'] in
      Lang.mem xind xid j id z n
    else false

def circleArea (cx cy rad : ℝ) : Set (ℝ × ℝ) := { (i, j) | (i - cx) ^ 2 + (j - cy) ^ 2 ≤ rad ^ 2 }

variable (yind xind xid yptr : loc) (N : ℕ) (M : ℕ) (K : ℕ)
variable (x_ind y_ind : ℤ -> ℝ) (x_id : ℤ -> ℤ) (y_ptr : ℤ -> ℤ) (out : loc)
variable (x_id_range : x_id '' ⟦0, N⟧ ⊆ Set.Ico 0 K)
variable (y_prt_mon : MonotoneOn y_ptr ⟦0, M+1⟧)
variable (y_ptr0 : y_ptr 0 = 0)
variable (y_ptrM : y_ptr M = N)
variable (y_ind_mono : StrictMonoOn y_ind ⟦0, M⟧)
variable (x_ind_mon : ∀ i ∈ ⟦0, M⟧, StrictMonoOn x_ind ⟦y_ptr i, y_ptr (i + 1)⟧)

variable (cx cy rad : ℝ) (rad_nonneg : 0 ≤ rad)

lemma circleArea_meaning_fix_x (x y : ℝ) :
  cx ≤ x + rad → x ≤ cx + rad →
  ((x, y) ∈ circleArea cx cy rad ↔ -√(rad ^ 2 - (x - cx) ^ 2) ≤ y - cy ∧ y - cy ≤ √(rad ^ 2 - (x - cx) ^ 2)) := by
  intros h1 h2
  simp [circleArea]
  trans ((y - cy) ^ 2 ≤ rad ^ 2 - ((x - cx) ^ 2))
  { rw [le_sub_iff_add_le'] }
  { rw [Real.sq_le] <;> simp
    apply sq_le_sq' <;> linarith }

include rad_nonneg in
lemma circleArea_outside_x (x y : ℝ) :
  cx + rad < x ∨ x + rad < cx → ¬ (x, y) ∈ circleArea cx cy rad := by
  intros h1
  have h : rad < x - cx ∨ x - cx < -rad := by cases h1 ; left ; linarith ; right ; linarith
  have h' : ¬ (x - cx) ^ 2 ≤ rad ^ 2 := by intro h_ ; apply abs_le_of_sq_le_sq' at h_ ; revert h ; simp at h_ ⊢ ; aesop
  simp at h'
  simp [circleArea]
  have := sq_nonneg (y - cy)
  linarith

include y_ind_mono

set_option maxHeartbeats 1600000 in
lemma mem2_spec_out (s : Set (ℝ × ℝ × ℤ)) :
  Disjoint s ((y_ind '' ⟦0, M⟧) ×ˢ ⋆) ->
  arr⟨⟪l,s⟫⟩(yind , x in M => y_ind x)  ==>
  WP [l| ijk in s => Lang.mem2 yptr yind xind xid ⟨ijk.val.1⟩ ⟨ijk.val.2.1⟩ ⟨ijk.val.2.2⟩] { v,
    ⌜v = fun _ => val_bool false⌝ ∗
    arr⟨⟪l,s⟫⟩(yind , x in M => y_ind x)  } := by
    move=> /Set.disjoint_left dj; ystep
    ywp; yapp Lang.findIdx_hspec_out=> //'
    { ystep; ywp; yifF=> //'; ywp; yval; ysimp=> //' }
    { sby apply StrictMonoOn.injOn }
    scase=> /== ? > ? /dj /==

set_option maxRecDepth 2000
set_option maxHeartbeats 20000000

attribute [-simp] Set.singleton_prod

include x_id_range y_ptr0 y_ptrM y_prt_mon
omit y_ind_mono in
lemma y_prt_range_j :
  j ∈ ⟦0, M⟧ ->
  x_id '' ↑(Finset.Ico (y_ptr j) (y_ptr (j + 1))) ⊆ Set.Ico 0 K := by
  simp only [Finset.mem_Ico, Finset.coe_Ico, and_imp]
  move=> ??; apply subset_trans; rotate_left=> //
  apply Set.image_mono=> ? /== ?? ⟨|⟩
  { apply le_trans'=> //; srw -y_ptr0
    apply y_prt_mon=> /== //' }
  apply Int.lt_of_lt_of_le=> //;
  apply le_trans (b := y_ptr (M ))=> //
  apply y_prt_mon=> /== //'

include x_ind_mon

-- omit x_id_range y_prt_mon y_ptr0 y_ptrM y_ind_mono x_ind_mon in
-- private lemma proj_box (v : hval (ℝ×ℤ)ˡ) :
--     0 ≤ i →
--     i < K →
--     y_ind j < y_box 1 ->
--     y_box 0 ≤ y_ind j ->
--       ({i_1 | (v ⟨2, (i_1, i)⟩).toBool = true} ∩ Set.Ico (x_box 0) (x_box 1)).ncard =
--         ({y_ind j} ×ˢ {i_1 | (v ⟨2, (i_1, i)⟩).toBool = true} ∩ Box y_box x_box).ncard := by
--   move=> > ?? ??; apply Set.ncard_congr (f := fun x _ => (y_ind j, x))=> /== //'
--   move=> > ->; simp [Box]=> -> //


def Set.Equiv (s₁ : Set α) (s₂ : Set β) :=
  ∃ f : α -> β,
  s₁.InjOn f ∧
  s₁.SurjOn f s₂ ∧
  ∀ x ∈ s₁, f x ∈ s₂

omit x_id_range y_prt_mon y_ptr0 y_ptrM y_ind_mono x_ind_mon in
lemma surj_on_ex_preim {α β} {s₁ : Set α} {s₂ : Set β} {f : α -> β} :
  s₁.SurjOn f s₂ -> ∀ y ∈ s₂, ∃ x ∈ s₁, f x = y := by
  move=> /[swap] y /(_ y) /[apply] //

omit x_id_range y_prt_mon y_ptr0 y_ptrM y_ind_mono x_ind_mon in
lemma equiv_card (s₁ : Set α) (s₂ : Set β) :
  Set.Equiv s₁ s₂ -> s₁.ncard = s₂.ncard := by
  scase=> f /== finj fsurj fIn; apply Set.ncard_congr (f := fun x _ => f x) => /== //'

omit x_id_range y_prt_mon y_ptr0 y_ptrM y_ind_mono x_ind_mon in
lemma equiv_finite (s₁ : Set α) (s₂ : Set β) :
  Set.Equiv s₁ s₂ -> (s₁.Finite <-> s₂.Finite) := by
  scase! => f fi fs fin
  refine Equiv.set_finite_iff ?intro.intro.intro.hst
  apply (Equiv.mk (fun ⟨x,pf⟩ => ⟨f x, fin x pf⟩)
    (fun ⟨y, pf⟩ => ⟨(choose (surj_on_ex_preim fs _ pf)), by move: (surj_on_ex_preim fs _ pf)=> x; move: (choose_spec x)=> //⟩))
  { move=>[x pf] /==; move: (surj_on_ex_preim fs _ (fin x pf))=> x
    scase: (choose_spec x)=> ? /fi // }
  scase=> x pf /==; move: (surj_on_ex_preim fs _ pf)=> x
  sby scase: (choose_spec x)

omit x_id_range y_prt_mon y_ptr0 y_ptrM y_ind_mono x_ind_mon in
lemma Set.biUnion_ncard (fs : Finset α) (s : α -> Set β) :
  (∀ i ∈ fs, (s i).Finite) ->
  (∀ᵉ (i ∈ fs) (j : fs), i ≠ j -> Disjoint (s i) (s j)) ->
  (⋃ i ∈ fs, s i).ncard = ∑ i in fs, ((s i).ncard : ℤ) := by
  induction fs using Finset.induction_on=> //' /==
  rename_i a fs nin ihfs=> fin /[dup] /ihfs {}ihfs fins dj djs
  srw Finset.sum_insert //' (Set.ncard_union_eq (hs := _) (ht := _)) /== //
  { move=> ? /[dup]? /dj; sapply=> ? // }
  move=> ?; apply (Set.Finite.biUnion')=> /==
  { srw (equiv_finite (s₂ := fs.toSet))=> //
    exists id=> ⟨? /== |⟨? /== //|//⟩⟩ }
  move=> ? /Set.mem_def //

include rad_nonneg

lemma circleCountQuery'_spec :
  { arr⟨⋆⟩(yind, x in M => y_ind x) ∗
    arr⟨⋆⟩(yptr, x in M + 1 => y_ptr x) ∗

    arr⟨⋆⟩(xind, x in N => x_ind x) ∗
    arr⟨⋆⟩(xid,  x in N => x_id x) ∗

    -- arr⟨⋆⟩(xbox, x in 2 => x_box x) ∗
    -- arr⟨⋆⟩(ybox, x in 2 => y_box x) ∗

    arr⟨⟪1, {r}⟫⟩(out, x in K => val_int 0) }
  [1| x in {r} => Lang.circleCountQuery' yptr yind xind xid cx cy rad out]
  [2| ijk in @Set.univ (ℝ × ℝ × ℤ) => Lang.mem2 yptr yind xind xid ⟨ijk.val.1⟩ ⟨ijk.val.2.1⟩ ⟨ijk.val.2.2⟩]
  {v,
    arr⟨⟪1, {r}⟫⟩(out, id in K => val_int ({(i, j) | v ⟨2,i,j,id⟩} ∩ circleArea cy cx rad).ncard)  ∗ ⊤ } := by
  shave seq : ∀ j (x : hval (ℝ×ℤ)ˡ) ,
      cy ≤ y_ind j + rad ->
      y_ind j ≤ cy + rad ->
      ∀ id ∈ ⟦0, K⟧, Set.Equiv
      ({y_ind j} ×ˢ {i | (x ⟨2, i, id⟩).toBool = true} ∩ circleArea cy cx rad)
      ({i | (x ⟨2, i, id⟩).toBool = true} ∩ (closedChordRange cx (rad ^ 2 - ((y_ind j) - cy) ^ 2)))
  { move=> j x ?? > ?; exists Prod.snd=> ⟨[]?? /== //|⟨?/== *| /== > ⟩⟩
    { exists (y_ind j) ; split_ands=> //
      srw circleArea_meaning_fix_x=> //; split_ands <;> linarith }
    move=> -> ? h ; srw circleArea_meaning_fix_x at h=> // ; simp at h ; rcases h with ⟨h, h'⟩
    split_ands=> // <;> try linarith
    apply sq_le_sq' <;> linarith }
  -- `seq₀` is implicitly used in some empty cases
  shave seq₀ : ∀ j (x : hval (ℝ×ℝ×ℤ)ˡ),
    cy + rad < y_ind j ∨ y_ind j + rad < cy ->
    ∀ id ∈ ⟦0, K⟧,
    ({y_ind j} ×ˢ {i | (x ⟨2, y_ind j, i, id⟩).toBool = true} ∩ circleArea cy cx rad) = ∅
  { move=> > ? > ? ! [??] /== -> _; apply circleArea_outside_x=> //' }
  shave Inj: Set.InjOn y_ind (Set.Ico 0 M)
  { sby apply StrictMonoOn.injOn }
  yfocus 2, (y_ind '' ⟦0, M⟧) ×ˢ ⋆
  -- yfocus 2, ⋆ ×ˢ (y_ind '' ⟦0, M⟧) ×ˢ ⋆
  yapp mem2_spec_out (y_ind_mono := y_ind_mono); rotate_left
  { exact Set.disjoint_sdiff_left }
  simp [fun_insert]
  yin 1: sdo 4 ystep; ywp; yref ind     -- !! this takes 50s
  srw [1]Set.image_eq_iUnion biUnion_prod_const
  let op id (hv : hval (ℝ×ℝ×ℤ)ˡ) i : ℤ := ({y_ind i} ×ˢ {j | hv ⟨2,y_ind i,j,id⟩} ∩ circleArea cy cx rad).ncard
  let P id (hv : hval (ℝ×ℝ×ℤ)ˡ) i := ({y_ind i} ×ˢ {j | hv ⟨2,y_ind i,j,id⟩} ∩ circleArea cy cx rad).Finite
  let cond (i : ℤ) := i < M ∧ y_ind i ≤ cy + rad
  ywhile+ 0, M with (b₀ := cond 0)
    (Inv := fun b i => (
        if cond i then
          ind ~⟪1, {r}⟫~> i
        else ∃ʰ (j : ℤ), (ind ~⟪1, {r}⟫~> j) ∗ ⌜0 <= j ∧ ¬ cond j⌝) ∗
            ⌜0 <= i ∧ i <= M ∧ cond i = b⌝)
    (Q := fun i hv =>
      (∑ j ∈ ⟦0, K⟧, (out + 1 + j.natAbs ~⟪1, {r}⟫~> op j hv i)) ∗
      ⌜∀ j ∈ ⟦0, K⟧, P j hv i⌝ )
    (H₀ := arr⟨⟪1, {r}⟫⟩(out, x in K => val_int 0)) <;> try simp [op, P]
  { move=> ?; split_ifs; simp [hhlocalE]; srw hhlocal_hhexists; simp [hhlocalE] } -- ? scase_if
  { move=> > ?? eq; congr! 10; congr! 1; all_goals srw eq //' }
  { move=> > ??
    yin 2:
      ystep; simpWP=> [_ [_ hcond]]
      (sdo 2 ystep=> //'); ywp; yifT=> /== //'
      srw if_pos //'
      ystep; ywp; yapp htriple_add; ystep=> //'
    yin 1:(sdo 3 ystep)
    yin 1: ywp; yif=> /== L₂
    { yin 1:ystep; ywp; yapp htriple_add; (sdo 4 ystep)=> //'
      srw !selfmul_is_square
      ysubst with (σ := Prod.snd)
      { ysimp }
      { move=> ?; split_ifs; simp [hhlocalE]; srw hhlocal_hhexists; simp [hhlocalE] } -- ? scase_if
      { move=> /== }
      zapp intervalCountQuery'_spec=> //'
      { apply y_prt_range_j (x_id_range := x_id_range)=> //' }
      { apply y_prt_mon=> /== //' }
      { srw -y_ptr0; apply y_prt_mon=> /== //' }
      { srw -y_ptrM; apply y_prt_mon=> /== //' }
      { apply x_ind_mon=> /== //' }
      move=> fin
      yapp
      move: hcond=> [_ hcond]
      ysimp[decide (cond (j + 1))]=> /== //'
      { move=> *; srw (equiv_finite _ _ (seq ..)) //' }
      { move=> *; srw (equiv_card _ _ (seq ..)) //' }
      split_ifs <;> ysimp; move=> ⟨|⟩ //' } -- ? scase_if
    yin 1: yapp=> fin
    yapp mem_spec_term; rotate_left
    { apply y_prt_mon=> /== //' }
    { srw -y_ptr0; apply y_prt_mon=> /== //' }
    { srw -y_ptrM; apply y_prt_mon=> /== //' }
    ysimp[decide (cond (j + 1))]=> /== //'
    split_ifs <;> ysimp; move=> ⟨|⟩ //' } -- ? scase_if
  { move=> > ? jM
    srw LGTM.triple ywp1; ypull=> /== _ _ hcond-- srw if_neg=> //' -- ? doing this here will break
    have hq : ¬ cond (j + 1) := by
      simp [cond] at hcond ⊢
      move=> h1; apply lt_trans; apply hcond=> //; apply y_ind_mono=> /== //'
    ystep; simpWP
    (sdo 2 ystep=> //'); ywp; yifT=> /== //'
    ystep; ywp; yapp htriple_add; ystep=> //'
    yapp mem_spec_term; rotate_left
    { apply y_prt_mon=> /== //' }
    { srw -y_ptr0; apply y_prt_mon=> /== //' }
    { srw -y_ptrM; apply y_prt_mon=> /== //' }
    srw ?if_neg //' /==
    simp [cond] at hcond; specialize hcond jM
    ypull=> k /== ??; ysimp=> //'
    split_ands=> // }
  { move=> > ??; split_ifs <;> rename_i L
    { ystep=> /== ?? hci; srw hci at L; subst b; simp at hci ; srw cond at hci
      ywp; yapp htriple_lt
      ywp; yifT=> //'; ystep; yapp htriple_ler
      ysimp=> /==  //' }
    srw -hwp_equiv; ypull=> i /== ? hh ?? hci; srw hci at L; simp at L; subst b; simp at hci
    ystep; ywp; yapp htriple_lt
    ywp; yif=> /== ?
    { ystep; yapp htriple_ler; ysimp=> /== //'; simp [cond] at hh; funext=> /== //' }
    ywp; yval; ysimp=> /== //' }
  { move=> ?; ypull; srw cond /== => ->; ysimp=> //' }
  { ysimp=> //; split_ifs <;> ysimp=> //' } -- ? scase_if
  ypull; srw cond=> /== > fin; ysimp=> /== //'
  move=> j ? hcondj i ?? /==
  srw -Set.biUnion_ncard /== //'
  { apply Set.ncard_congr  (f := fun x _ => x)=> /== >
    { move=> *; scase_if=> /== //'; { move=> * ⟨|⟩ // }
      exists x=> //' }
    scase_if=> /== > *; exists x=> ⟨//|⟩//' }
  move=> > *
  apply Disjoint.inter_left
  apply Disjoint.inter_right=> /==; left
  move=> /(StrictMonoOn.injOn y_ind_mono) /== //'
