...
lemma LGTM.wp_for_bighop (β : Type) [inst : Inhabited β]
  ...
  s = ∑ i in [[z, n]], sᵢ i ->
  (∀ (hv : hval), ∀ j ∈ [[z,n]], ∃ v, H₀ + ∑ i in [[z, j]], Q i hv = Qgen j v) ->
  Disjoint s' s ->
  (∀ i j, i != j -> z <= i ∧ i < n -> z <= j ∧ j < n -> Disjoint (sᵢ i) (sᵢ j)) ->
  (∀ j v, z <= j ∧ j < n ->
  ...
  { move=> j hv ?
    specialize gen hv=> //'
    move: gen=> /(choose_fun (b₀ := default))/(_ inst)
    scase=> hv' /== /[dup] gen -> //'
    srw //' [2]sum_Ico_succl //' hhProp_add_def
    ychange ind=> //'; apply hhimpl_trans; apply LGTM.wp_frame; apply hwp_conseq=> hv' /=
    srw -gen //'
    srw [5]sum_Ico_predr //' /== ?hhProp_add_def
    ysimp;
    srw [2]sum_Ico_predr //' /== hhProp_add_def hhstar_comm
    srw eqQ; ysimp=> //'; srw Finset.sum_congr; apply hhimpl_refl=> //'
    move=> /== k ??; apply eqQ=> //' ? f /==
...
lemma LGTM.wp_while_bighop (β : Type) [inst : Inhabited β]
  ...
  { move=> j hv ?
    ...
    apply eqQ=> //' }
  { move=>> ?
    srw -hhstar_assoc -hhstar_assoc
    srw LGTM.hwp_Q_eq; rotate_left 2
    { move=> ?
      rewrite [sum_bighstar, bighstar_hhstar, bighstar_hhstar, <-hhstar_assoc]
      rfl=> //' }
    apply hhimpl_trans_r; apply hwp_frame; ysimp
    rewrite [sum_bighstar, bighstar_hhstar, bighstar_hhstar]
    apply htriple_prod_val_eq=> a /cndE
    srw -?Unary.wp_equiv=> {}cndE
    xchange cndE=> //'; apply himpl_trans; apply Unary.wp_frame; apply Unary.wp_conseq=> ? /==
    srw qstarE /=; xsimp }
  { move=> hv /= b;
    move: (cndn b)=> /Disjoint.hhimpl_bighstar_himpl imp
    apply (@ychange_lemma _ ((fun i ↦ hpure (b = false) ∗ Inv b n i) ∗↑ s'))
    apply imp; ysimp; srw -bighstar_hhstar bighstar_hpure_nonemp //'
    ysimp }
  { srw Finset.Ico_self /== -sum_bighstar_set; ysimp; ysimp }
...
lemma wp_for_bighop_aux (β : Type)  [inst : Inhabited β]
  ...
  z <= n ->
  (∀ j hv₁ hv₂, z <= j ∧ j < n -> (∀ x, x ∈ sᵢ j -> hv₁ x = hv₂ x) -> (Q j hv₁) = (Q j hv₂)) ->
  (∀ (hv : Int -> hval), ∀ j ∈ [[z,n]], ∃ v, H₀ + ∑ i in [[z, j]], Q i (hv i) = Qgen j v) ->
  (∀ j (v : β), z <= j ∧ j < n ->
    Qgen j v ∗ Inv j ∗ (R ∗↑ (sᵢ j)) ==>
      LGTM.wp
           [⟨s', fun _ => subst vr j c⟩, ⟨sᵢ j, ht⟩]
  ...
  { move=> hv j /gen /(_ (fun i => hv ∘ (i, ·)))[v] eq
    exists v; srw -eq ?hhProp_add_def -hsubst_hhstar_same //'
...
lemma wp_for_bighop [Inhabited α] (β : Type)  [inst:Inhabited β]
  ...
  (∀ i b, hhlocal s' (Qgen i b)) ->
  (∀ j hv₁ hv₂, z <= j ∧ j < n -> (∀ x, x ∈ sᵢ j -> hv₁ x = hv₂ x) -> (Q j hv₁) = (Q j hv₂)) ->
  (∀ (hv : Int -> hval), ∀ j ∈ [[z,n]], ∀ a ∈ s', ∃ v, H₀ + ∑ i in [[z, j]], Q i (hv i) = Qgen j v) ->
  (∀ j (v : α -> β), z <= j ∧ j < n ->
    Qgen j (v i) ∗ Inv j ∗ (R ∗↑ (sᵢ j)) ==>
      LGTM.wp
           [⟨s', fun _ => subst vr j c⟩, ⟨sᵢ j, ht⟩]
           (fun hv' =>
             Q j hv' + Qgen j (v i) ∗ Inv (j+1) ∗ R' ∗↑ sᵢ j)) ->
  H₀ ∗ Inv z ∗  R ∗↑ s ==>
    LGTM.wp
         [⟨s', fun _ => trm_for vr z n c⟩, ⟨s, ht⟩]
         (fun hv => H₀ + (∑ j in [[z, n]], Q j hv) ∗ Inv n ∗ R' ∗↑ s) := by
  ...
  srw Function.comp /=
  srw ForLoopAux.LGTM.triple_Q_eq; rotate_left
  { move=> ?;
    rewrite [<-bighstar_hhstar, <-Disjoint.sum_bighstar]
    rfl }
  apply ForLoopAux.wp_for_bighop_aux _ _ _
    (df := none)
    (fun i => ssubst some (s' ∪ s) $ sᵢ i)
    (Qgen := fun i v a => Qgen i v a.get!)
    (Inv := fun i a => Inv i a.get!)
    (inst := inst)
    (Q := fun j hv a => Q j (hv ∘ some) a.get!)=> //'
  { move=> j hv₁ hv₂ [] /== a _ ? ?? hveq; apply eqQ=> //'
    move=> x ?; apply hveq=> //'; right; srw seq /==; exists j=> //' }
  { move=> ??? [] /== a _ //' }
  { move=> > ?
    shave ?: Disjoint s' (sᵢ j)
    { move: seq disj=> ->; srw -disjoint_union=> /== // }
    shave ?: ∀ a ∈ ssubst some (s' ∪ sᵢ j) s', Qgen j (v a) a.get! = Qgen j (v (some a.get!)) a.get!
    { move=> [] /== }
    srw (lem_s' s z n sᵢ) //' (lem_s s z n sᵢ) //' bighstar_eq
    { srw [2]bighstar_eq
      rw [<-(hsubst_some_hhstar' _ s' (sᵢ j)
        (H₁ := fun i ↦ Qgen j (v i) i)
        (H₂ := fun i ↦ Inv j i)
        (H₃ := fun i ↦ R i))]=> //'
      srw LGTM.wp_Q_eq; rotate_left 2
      { move=> hv
        rewrite [hhProp_add_def, bighstar_hhstar]
        srw -(hsubst_some_hhstar' (H₁ := fun i => Q j _ i ∗ Qgen j (v i) i)) //' }
      apply hsubst_wp=> //'
      { move=> * [] // }
      { move=> > hveq; congr; apply bighstar_eq=> ??; apply eqQ=> //' ??
        apply hveq=> //' }
      move=> ?; srw -bighstar_hhstar hhProp_add_def }
    sdone }
...
--   bigUnion : shts.set = ∑ i in [[z, n]], sᵢ i
...
--   setIsBigUnion z n
--     (⟨∑ i in [[z,n]], sᵢ' i, ht⟩ :: shts)
--     (fun i => sᵢ i ∪ sᵢ' i) where
--   bigUnion := by
--     srw /== h.bigUnion -set_plusE -Finset.sum_add_distrib
--     apply Finset.sum_congr=> // *; srw Set.union_comm //

-- class htrmIsBigUnion (shts : LGTM.SHTs α) (sᵢ : outParam htrm) where
